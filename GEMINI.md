...
31. **Product Recommendation Agent & Merchandising Skill**: Added `recommendation_pipeline` (SequentialAgent combining `recommendation_reasoner` and `recommendation_formatter`) powered by `skills/product-recommender/SKILL.md` and `ProductRecommendationSchema` in [`app/schemas.py`](file:///Users/henrikw/Projects/agent_platform_demo/app/schemas.py). The agent analyzes segment attributes from BigQuery (`customer_demographics_360`, `customer_rfm_summary`, `customer_transactions`) and curates exactly 5 tailored items from `product_catalog` with EUR pricing, sustainability badges, and data-driven rationale. Added dedicated Product Recommendation UI card rendering in [`ChatInterface.jsx`](file:///Users/henrikw/Projects/agent_platform_demo/frontend/src/components/ChatInterface.jsx), updated dynamic AI follow-up suggestions in [`backend/suggestions_generator.py`](file:///Users/henrikw/Projects/agent_platform_demo/backend/suggestions_generator.py), added evaluation cases (`eval_19`, `eval_20`, `eval_07`), and updated all architecture specs and diagrams.
